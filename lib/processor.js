...
exports.uploadAndCopy = (file) => {
  ...
  if (file.s3Bucket && file.s3Key) {
    uploadOriginal = this.copy(file.s3Bucket, file.s3Key, file.path, file.name);
  } else {
    uploadOriginal = this.upload(file.localPath, file.path, file.name);
  }
  ...
  return uploadOriginal.then(data => {
    let makeCopies = fileCopies.map(n => this.copy(data.Bucket, data.Key, file.path, n));
...
exports.copy = (fromBucket, fromKey, filePath, fileName) => {
  return Q.ninvoke(s3, 'copyObject', {
    CopySource: `${fromBucket}/${fromKey}`,
    Bucket: process.env.DESTINATION_BUCKET,
    Key: `${filePath}/${fileName}`
  }).then(() => {
    return {Bucket: process.env.DESTINATION_BUCKET, Key: `${filePath}/${fileName}`};
  });
}
